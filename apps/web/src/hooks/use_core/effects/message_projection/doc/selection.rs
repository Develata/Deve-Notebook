...
pub(crate) fn settle_pending_document_create(
    docs: &[(DocId, String)],
    signals: CoreSignals,
) -> bool {
    let Some(pending) = signals.pending_document_create.get_untracked() else {
        return false;
    };
    let Some(projected) = pending.observe_docs(docs) else {
        return false;
    };
    if projected.select && signals.current_doc.get_untracked().is_none() {
        signals.set_current_doc.set(Some(projected.doc_id));
    }
