//! Merge intents for a backend-projected conflict draft.
//! plan_ref:
...
#[component]
pub(super) fn MergeConflictActions(
    mobile: bool,
    conflict: MergeConflictSession,
    resolved_content: ReadSignal<String>,
    ...
    view! {
        <div class=move || if mobile {
            ...
            <span class="text-xs font-semibold text-[var(--diff-fg)]">{move || t::diff::merge_conflict(locale.get())}</span>
            <Show when=move || has_current>
                <button class=button_class(mobile) data-deve-merge-action="accept-current" on:click=move |_| on_resolve.run((MergeConflictAction::AcceptCurrent, None))>
                    {move || t::diff::accept_current(locale.get())}
                </button>
            </Show>
            <Show when=move || has_incoming>
                <button class=button_class(mobile) data-deve-merge-action="accept-incoming" on:click=move |_| on_resolve.run((MergeConflictAction::AcceptIncoming, None))>
                    {move || t::diff::accept_incoming(locale.get())}
                </button>
            </Show>
            <Show when=move || has_both>
                <button class=button_class(mobile) data-deve-merge-action="accept-both" on:click=move |_| on_resolve.run((MergeConflictAction::AcceptBoth, Some(resolved_content.get_untracked())))>
