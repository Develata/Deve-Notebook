...
pub fn UnstagedSection(unstaged: Vec<ChangeEntry>) -> impl IntoView {
    ...
    Effect::new(move |_| {
        let count = core.unstaged_changes.get().len();
        ...
        <div>
            ...
            {move || if expanded.get() {
                view! {
                    ...
                    />
                    <Show when=move || { unstaged_count > render_limit.get() }>
                        <button
                            class="w-full py-1 text-[11px] text-blue-600 hover:bg-blue-50"
                            on:click=move |_| set_render_limit.update(|v| *v += 200)
                        >
                            {move || t::source_control::show_more(locale.get(), unstaged_count.saturating_sub(render_limit.get()))}
                        </button>
                    </Show>
                }.into_any()
            } else {
