<!-- Parent: ../AGENTS.md -->
<!-- Generated: 2026-03-22 -->

# shortcuts

## Purpose

Keyboard shortcut system. Defines shortcuts, manages a global registry, and handles key event dispatch.

## Key Files

| File | Description |
|------|-------------|
| `mod.rs` | Shortcut module entry |
| `config.rs` | Shortcut configuration and default bindings |
| `global.rs` | Global keyboard event listener |
| `registry.rs` | Shortcut registry — maps key combos to actions |
| `types.rs` | Shortcut type definitions |

## For AI Agents

### Working In This Directory

- See `12_commands.md` in deve-note plan for keyboard shortcut design.

<!-- MANUAL: -->
