...
use super::super::transport::{S3HttpResponse, S3StreamResponse, S3Transport};
...
pub(super) fn s3_list_body_with_state(
    keys: &[&str],
    is_truncated: bool,
    ...
    body.push_str(if is_truncated { "true" } else { "false" });
    body.push_str("</IsTruncated>");
    for key in keys {
