use crate::remote_projection_transport::collect_markdown_projection_files;
use std::fs;
...
fn collect_markdown_projection_files_uploads_only_markdown_projection_files() {
    ...
    fs::create_dir_all(dir.path().join("SNAPSHOTS")).expect("snapshots");
    ...
    fs::write(dir.path().join("SNAPSHOTS").join("local.md"), "snapshots").expect("snapshots file");
