...
use super::merge_peer_test_support::{
    MergeConflictExpectation, browser_remote_session, doc_content, doc_entry_count,
    ensure_remote_repo, expect_merge_complete, expect_merge_conflict, local_doc_content,
    local_doc_entry_count, reopen_state, request_merge_peer, seed_local_doc, seed_local_replace,
    seed_remote_replace, seed_shared_base,
};
...
async fn merge_peer_conflict_replays_after_state_reopen_without_losing_ops() -> anyhow::Result<()> {
    ...
    let mut interrupted = browser_remote_session(&peer_id, repo_id, 61);
    ...
    let mut resumed = browser_remote_session(&peer_id, repo_id, 62);
