...
async fn edit_clears_stale_remote_readonly_binding_before_checks() -> anyhow::Result<()> {
    ...
    let stale_db = Arc::new(redb::Database::create(h.dir.path().join("stale-remote.redb"))?);
    let (ch, mut uni_rx) = unicast_channel(&h.state);
    let mut session = writer_browser_session("default", h.default_repo_id, 29);
...
async fn edit_rejects_stale_message_scope_at_writer_gate() -> anyhow::Result<()> {
    ...
    let (ch, mut uni_rx) = unicast_channel(&h.state);
    let mut session = writer_browser_session("default", h.default_repo_id, 42);
