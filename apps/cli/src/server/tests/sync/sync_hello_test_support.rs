...
use super::{
    channel::DualChannel, handlers::sync::SyncHelloInput, security, session::WsSession,
    tree_state::RepoTreeRegistry, AppState,
};
...
pub(super) fn signed_hello_for_repo(
...
pub(super) fn empty_session() -> WsSession {
    WsSession::new()
}

pub(super) fn block_shadow_peer_dir(
    state: &Arc<AppState>,
    remote: &IdentityKeyPair,
) -> anyhow::Result<()> {
