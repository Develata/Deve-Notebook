...
- case_id: REL-003
  ...
  preconditions:
    - CI 环境
    - Redb v4、F4/v3 与 immutable Remote Import backend/product cutover 已实现；B5/B6 与其余 release gates 仍阻塞 tag
  ...
  steps:
    ...
    - run: rg -n "WS_PROTOCOL_VERSION = 3" docs/registry/first-tag-format-matrix.md
    - run: rg -n "已对齐：代码为 F4/v3 lockstep" docs/registry/first-tag-format-matrix.md
