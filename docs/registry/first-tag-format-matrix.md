...
| WebSocket binary protocol | `docs/plan/07_network.md#server-ws-runtime` | `WS_PROTOCOL_VERSION = 13;`; `MIN_SUPPORTED_WS_PROTOCOL_VERSION = 13;`; magic `DEVEWSF3`; project-owned postcard frame payload; Diff uses typed projections and on-demand commit-file requests | `crates/core/src/protocol/frame.rs` | Missing binary magic or unsupported protocol version fails closed with structured protocol error. First-tag policy is lockstep `13..=13`; v12 has no adapter and lowering minimum without adapters is not compatibility. |
