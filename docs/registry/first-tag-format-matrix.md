...
| Remote Projection transport / Remote Import | `docs/plan/06_backup.md#remote-projection-transport-contract`; `docs/plan/06_backup.md#remote-import-session-contract` | `projection-remote <provider> push` + immutable source acquisition → Remote Import session/review/sealed Ledger Apply；host-local profile binding，不暴露 secret/path/digest | 部分对齐：B1 durable immutable session/artifact/candidate/recovery/repair store 已实现；provider extraction、sealed Apply、产品切换与 typed client 仍由 B2-B6 完成，旧 pull drift 仍阻塞 tag。 | `crates/core/src/remote_projection/`; `crates/core/src/remote_import/`; `apps/cli/src/commands/projection_remote.rs` | Locator/profile/provider/path/budget failure must precede authority effects。Prepare 只封存 immutable blobs；任意 blocker 禁用 whole-session Apply；authority transaction 持久化 Pending receipt，post-commit writeback 后单调收敛为 Written/Degraded，不回滚或重复写 Ledger。 |
...
Implementation status after B1: Ledger payload v3, Redb schema v4 and Projection
Locator are aligned. The durable Remote Import store is present, while F4/v3 and
the provider/product/apply/client portions remain pending. The matrix therefore
continues to block tag readiness until B2-B6 replace every remaining target/current
