...
| `remote_projection_transport_runtime` | `部分承载` | `crates/core/src/remote_projection/`; `apps/cli/src/commands/projection_remote.rs`; `apps/cli/src/commands/projection_remote/webdav/`; `apps/web/src/components/command_palette/registry/remote_projection.rs` | `待分配` | WebDAV/S3 Markdown Projection Workspace push/pull；CLI WebDAV push/pull 已接入 provider adapter；pull 只覆盖 projection files 并经 watcher/scan 进入 External Changes，不得写 ledger、commit anchor、Source Control staging 或 Git main mirror queue。 |
