...
| `backup_restore_runtime` | `部分承载` | `crates/core/src/backup/branch_manifest.rs`; `crates/core/src/backup/verification.rs`; `crates/core/src/backup/restore.rs`; `crates/core/src/backup/restore_flow.rs`; `apps/cli/src/commands/backup/restore.rs`; `apps/cli/src/commands/backup/provider_io.rs`; `apps/cli/src/commands/backup/provider_io/` | `待分配` | branch.manifest.enc expected digest/routing/AEAD authentication 后生成 typed branch manifest，pack provider download 与 pack open/decrypt 都由该 manifest 的 pack refs 驱动；RestoreCandidate admission 只能从 typed manifest verification、PacksDecrypted evidence 与 core-owned resource budget 派生；backup-owned provider download 只返回 encrypted artifact bytes 与 diagnostic-only metadata，不能成为 authority；remote-readonly candidate 仅为内存态，不写 local branch、ledger、Source Control 或 Projection Workspace。 |
