...
| `document_client` | `部分承载` | `apps/web/src/runtime/document_client/`; `apps/web/src/runtime/document/`; `apps/web/src/editor/sync/`; `apps/web/src/editor/hook_runtime.rs` | `docs/tasks/20_web_thin_client_ledger_migration.md` | Browser document snapshot、pending overlay、ack/reject、失败原子 replay 与 navigation guard；client-side coordination adapter，adapter/replay 失败保持只读并要求新 generation/request Retry。 |
