...
| `backup_restore_runtime` | `部分承载` | `crates/core/src/backup/verification.rs`; `crates/core/src/backup/restore.rs`; `crates/core/src/backup/restore_flow.rs`; `apps/cli/src/commands/backup/restore.rs`; `apps/cli/src/commands/backup/provider_io.rs`; `apps/cli/src/commands/backup/provider_io/` | `待分配` | verify/decrypt 后生成 verification result、restore flow 与 candidate admission metadata；RestoreCandidate admission 只能从 typed manifest verification 与 PacksDecrypted evidence 派生；backup-owned provider download 只返回 encrypted artifact bytes 与 diagnostic-only metadata，不能创建 restore candidate；CLI 只暴露 dry-run flow planning，不得直接写 local branch 或 Projection Workspace。 |
