...
| `external_changes_runtime` | `部分承载` | `crates/core/src/source_control/`; `crates/core/src/ledger/manager/commit_runtime.rs`; `apps/cli/src/server/handlers/source_control/` | `待分配` | Projection Workspace 外部偏差的 read/stage/unstage/discard/apply-to-ledger；不得创建 commit anchor、history、graph 或 Git mirror state。 |
...
| `external_changes_client` | `部分承载` | `apps/web/src/runtime/external_changes_client/`; `apps/web/src/hooks/use_core/contexts/external_changes.rs`; `apps/web/src/components/sidebar/external_changes.rs` | `待分配` | Browser External Changes typed intent facade；只展示外部投影偏差并发送 stage/unstage/discard/apply-to-ledger intent，不拥有 ledger/source-control authority。 |
