...
| `backup_pack_runtime` | `部分承载` | `crates/core/src/backup/pack.rs`; `crates/core/src/backup/artifact.rs`; `crates/core/src/backup/protection.rs`; `crates/core/src/backup/upload.rs`; `apps/cli/src/commands/backup/run.rs` | `待分配` | 从 ledger/snapshot authority 规划 backup pack manifest、加密 pack artifact、artifact protection metadata 与 upload state admission；encrypted artifact open 必须 verify-before-decrypt；CLI 只暴露 dry-run 计划，不得读取 stale UI state、直接访问 provider 或上传。 |
