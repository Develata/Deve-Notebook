...
| `backup_restore_runtime` | `部分承载` | `crates/core/src/backup/branch_manifest.rs`; `crates/core/src/backup/verification.rs`; `crates/core/src/backup/restore.rs`; `crates/core/src/backup/restore_flow.rs`; `apps/cli/src/commands/backup/restore.rs`; `apps/cli/src/commands/backup/provider_io.rs`; `apps/cli/src/commands/backup/provider_io/` | `待分配` | branch.manifest.enc expected digest/routing/AEAD authentication 后生成 typed branch manifest，pack provider download 由该 manifest 的 pack refs 驱动；RestoreCandidate admission 只能从 typed manifest verification 与 PacksDecrypted evidence 派生；backup-owned provider download 只返回 encrypted artifact bytes 与 diagnostic-only metadata，不能创建 restore candidate；CLI 不得直接写 local branch 或 Projection Workspace。 |
