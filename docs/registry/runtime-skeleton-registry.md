...
| `external_changes_client` | `部分承载` | `apps/web/src/runtime/external_changes_client/`; `apps/web/src/hooks/use_core/contexts/external_changes.rs`; `apps/web/src/components/sidebar/external_changes.rs` | `docs/tasks/19_repo_refactor_blueprint.md` | Browser External Changes typed intent facade；只展示外部投影偏差并发送 stage/unstage/discard/apply-to-ledger intent，不拥有 ledger/source-control authority。 |
