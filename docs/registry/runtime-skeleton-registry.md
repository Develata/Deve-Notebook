...
| `backup_locator_runtime` | `部分承载` | `crates/core/src/backup/locator.rs`; `crates/core/src/backup/provider.rs`; `crates/core/src/backup/root.rs`; `crates/core/src/backup/layout.rs`; `crates/core/src/backup/discovery.rs`; `crates/core/src/backup/secret.rs`; `crates/core/src/backup/binding.rs`; `crates/core/src/backup/binding_store.rs`; `apps/cli/src/commands/backup.rs`; `apps/cli/src/commands/backup/bind.rs`; `apps/cli/src/commands/backup/unbind.rs` | `待分配` | repo/branch URL 到 WebDAV/S3 backup locator、provider adapter dispatch、backup root manifest、remote layout diagnostics、readonly branch discovery、credential/key ref、branch binding 的解析与 host-local secret-free bind/unbind persistence；不得成为 repo authority。 |
| `backup_pack_runtime` | `部分承载` | `crates/core/src/backup/pack.rs`; `crates/core/src/backup/artifact.rs`; `crates/core/src/backup/artifact/error.rs`; `crates/core/src/backup/artifact/types.rs`; `crates/core/src/backup/artifact/validation.rs`; `crates/core/src/backup/artifact/verification.rs`; `crates/core/src/backup/protection.rs`; `crates/core/src/backup/upload.rs`; `apps/cli/src/commands/backup/run.rs`; `apps/cli/src/commands/backup/provider_io.rs`; `apps/cli/src/commands/backup/provider_io/` | `待分配` | 从 ledger/snapshot authority 规划 backup pack manifest、加密 pack artifact、artifact protection metadata、upload state admission 与 backup-owned provider upload；encrypted artifact open 必须 verify-before-decrypt；非 dry-run CLI 只能上传显式提供且已通过 manifest/digest 校验的 encrypted artifact bytes，provider PUT 后必须读回同一 remote object 并校验 readback digest 后才进入 `RemoteVerified`，不得读取 stale UI state、复用 Remote Projection transport、写 ledger/staging/commit anchor 或 Projection Workspace。 |
| `backup_restore_runtime` | `部分承载` | `crates/core/src/backup/branch_manifest.rs`; `crates/core/src/backup/branch_manifest/artifact.rs`; `crates/core/src/backup/branch_manifest/artifact/error.rs`; `crates/core/src/backup/branch_manifest/validation.rs`; `crates/core/src/backup/plaintext.rs`; `crates/core/src/backup/verification.rs`; `crates/core/src/backup/restore.rs`; `crates/core/src/backup/restore_flow.rs`; `apps/cli/src/commands/backup/restore.rs`; `apps/cli/src/commands/backup/restore/download.rs`; `apps/cli/src/commands/backup/restore/download/validation.rs`; `apps/cli/src/commands/backup/provider_io.rs`; `apps/cli/src/commands/backup/provider_io/` | `待分配` | remote-readonly provider restore path 已由 CLI/core/provider I/O 承载：branch.manifest.enc expected digest/routing/AEAD authentication 后生成 typed branch manifest，pack provider download 与 pack open/decrypt 都由该 manifest 的 pack refs 驱动；RestoreCandidate admission 只能从 typed manifest verification、PacksPlaintextVerified evidence 与 core-owned resource budget 派生；backup-owned provider download 只返回 encrypted artifact bytes 与 diagnostic-only metadata，不能成为 authority；remote-readonly candidate 仅为内存态，不写 local branch、ledger、Source Control 或 Projection Workspace。`explicit-import` / `explicit-merge` 仍在 writer gate 与 RestoreCandidate import/merge authority 链路落地前 fail-closed，首版前需要单独决策和实现。 |
| `remote_projection_transport_runtime` | `部分承载` | `crates/core/src/remote_projection/`; `apps/cli/src/commands/projection_remote.rs`; `apps/cli/src/commands/projection_remote/webdav/`; `apps/cli/src/commands/projection_remote/s3/`; `apps/web/src/components/command_palette/registry/remote_projection.rs` | `待分配` | WebDAV/S3 Markdown Projection Workspace push/pull；CLI WebDAV push/pull 与 AWS `s3://` S3 push/pull 已接入 provider adapter；S3 custom endpoint 仍 fail-closed，等待显式 credential binding；pull 只覆盖 projection files 并经 watcher/scan 进入 External Changes，不得写 ledger、commit anchor、Source Control staging 或 Git main mirror queue。 |
...
| `native_shell_mode_runtime` | `部分承载` | `crates/core/src/native_adapter/`; `apps/desktop/src/service_entrypoint/`; `apps/desktop/src/tauri_bootstrap/`; `apps/mobile/src/tauri_entry.rs`; `apps/mobile/src/embedded_backend/`; `apps/cli/src/native_runtime.rs`; `apps/web/src/api/native_backend_bridge.rs`; `apps/web/js/native_backend_bridge.js` | `待分配` | Desktop/Android/Mobile `LocalBackend` 与 `RemoteBrowser` 模式选择、URL 验证、本地 loopback/embedded 后端装配入口；Web facade 只暴露 host-local backend preference intent；shell 不直接写 authority。 |

## Notes

- `source_control_runtime -> Git main mirror bridge` 仍以 `docs/plan/05_diff_logic.md#git-mirror-lifecycle` 为权威边界；NoteGit/ngit 是唯一 Source Control authority，Git main mirror 不再由 `source_control.git_bridge` 配置切换。
- `remote_projection_transport_runtime` 独立于 `backup_*_runtime`；WebDAV/S3 projection transport 不复用加密 backup pack authority，也不得被当作同步 authority。
