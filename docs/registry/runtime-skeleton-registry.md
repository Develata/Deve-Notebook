...
| `native_shell_mode_runtime` | `部分承载` | `crates/core/src/native_adapter/`; `apps/desktop/src/service_entrypoint/`; `apps/desktop/src/service_entrypoint/git_executable.rs`; `apps/desktop/src/tauri_bootstrap/`; `apps/mobile/src/tauri_entry.rs`; `apps/mobile/src/embedded_backend/`; `apps/cli/src/native_runtime.rs`; `apps/web/src/api/native_backend_bridge.rs`; `apps/web/js/native_backend_bridge.js`; `scripts/check-desktop-packaged-ui-smoke.ps1`; `scripts/smoke-desktop-packaged-ui.mjs` | `待分配` | Desktop/Android/Mobile `LocalBackend` 与 `RemoteBrowser` 模式选择、URL 验证、本地 loopback/embedded 后端装配入口；Desktop 向 sidecar 二选一传递 host 验证后的 canonical Git executable 或 unavailable marker，不传完整 PATH；Windows packaged gate 驱动已安装真实 WebView 并证明 sidecar cleanup，仍只提交 UI intent；Web facade 只暴露 host-local backend preference intent；shell 不直接写 authority。 |
