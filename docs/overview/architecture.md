...
- `LocalAuthorityRuntime` is the sole per-RepoId Redb owner. Non-clone leases borrow the database; product mutation admission closes before provider quiesce and watcher E2, then Quiescing drains bounded in-flight use. The per-RepoId lock pathname is persistent host coordination identity; its OS handle stays held through exact cleanup/tombstone retirement. R4 implements `CommittedCleanup -> Retiring(process-local release reservation) -> Retired`, releasing the final OS lock outside the slot mutex before the exact Retired CAS. Option A now implements `Retired(expected lock identity) -> Reopening -> ReopeningPrepared -> fresh Normal -> guarded Active(next generation)`; prepared identity v2 binds DB, locator store/row, workspace root/marker and lock, and the short activation order is locator capability -> catalog membership guard -> authority slot. No UI or catalog component owns those resources.
...
- Watcher bootstrap enumerates durable healthy RepoIds without pre-resolving per-repo metadata. Backend/scan/worker/panic failures settle only that RepoId as `Failed`, and the failure callback atomically closes mount admission before cleanup; the typed host-fatal set (`SupervisorInvariant`, `GenerationCorruption`, `ThreadResourceExhaustion`, `RuntimeCoordinationFailure`) rolls back all started handles. Zero Mounted repos still leave the host available for readonly, diagnostics and Create; default scope and writer registration exclude non-Mounted repos.
- A fallback is an optional user choice bound during Prepare, never backend auto-selection. The approved R4 target persists a non-publishable TerminalCandidate, retires/releases authority, then enables terminal publication. R5 still owns the single typed RepoList/scope finalization.
