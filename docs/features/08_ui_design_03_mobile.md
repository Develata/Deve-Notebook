...
- 左侧 drawer 的关闭按钮应表达关闭侧栏，而不是在 Source Control、Search、Extensions 等视图中仍标成关闭文件树。
- 关闭状态的 drawer 不应向可访问树或交互树暴露离屏按钮，避免 Source Control / Search 等入口被误触发。
...
- 底部面板中的文档/差异行与关闭按钮应在可访问名称中包含对应标题，避免多个已打开 surface 在读屏或自动化快照中不可区分。
- 底部面板打开后应以可命名 dialog 语义呈现，键盘焦点限制在面板内，并支持 Escape 关闭。
