...
- Android target-host 在注入每次真实 Drawer swipe、首次文档 Create native touch 或用于归一化已打开 Drawer 的系统 Back 前，都必须等待当代 WebView 文档可见、持续取得页面焦点、精确应用 Activity 保持 `resumed`，并由 WindowManager `mCurrentFocus` 与 InputDispatcher 当前 focused window 形成精确、无冲突的同应用输入焦点共识；一个精确探针只可在另一个因命令失败、缺失或平台明确 `null` / `<none>` 而 soft-unavailable 时提供结论。两个可用探针冲突、两个都 soft-unavailable，以及任一探针出现历史快照替代当前状态、负 display ID、不可解析记录、package 前缀碰撞或其它 hard-invalid 格式/身份歧义都必须阻断，不能由另一条精确结果覆盖。CDP 可连接、presentation ready、writer ready、目标控件可见、仅有 `document.hasFocus()` 或仅有 resumed Activity 都不能替代 Android 当前输入目标准入。页面 reload、Activity/窗口状态冲突或采样中断后必须重新累计稳定窗口；只有 LocalBackend Drawer swipe/Back lane 的短时被动准入失败时，才可在精确 PID 保持不变的前提下执行至多一次 Launcher foreground reentry，然后从零重新准入，但不得重建业务 surface 或重发任何 swipe/Back。Create lane 与 RemoteBrowser HTTPS WebView 只能等待完整被动准入窗口，不得通过 Launcher reentry 扰动已绑定 Document。一次 missing/cancelled swipe retry 也必须重新准入，且没有完整 WebView 触摸事件时仍然失败。Drawer Back 通过页面内一次性 token observation 区分 ADB transport 与 Web 语义：只有 keyevent 完全未到达 Web 且 Drawer 仍 open 时才可重新准入并补发至多一次；一旦 Web 已看到 Back 就不得重发，也不得以重试掩盖错误 surface 消费或无效 ack。
