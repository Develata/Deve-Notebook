...
- Android target-host 在注入每次真实 Drawer swipe、首次文档 Create native touch 或用于归一化已打开 Drawer 的系统 Back 前，都必须等待当代 WebView 文档可见且持续取得输入焦点；CDP 可连接、presentation ready、writer ready 或目标控件可见不能替代该输入准入。页面 reload 或采样中断后必须从新文档重新累计稳定窗口；一次 missing/cancelled swipe retry 也必须重新准入，且没有完整 WebView 触摸事件时仍然失败。Drawer Back 通过页面内一次性 token observation 区分 ADB transport 与 Web 语义：只有 keyevent 完全未到达 Web 且 Drawer 仍 open 时才可重新准入并补发至多一次；一旦 Web 已看到 Back 就不得重发，也不得以重试掩盖错误 surface 消费或无效 ack。
