...
- `Ack.seq` and `NewOp.entry.seq` use the same repo-global ledger sequence as snapshot/history versions; peer-local fact sequence is never a browser document version.
