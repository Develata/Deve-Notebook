...
- `Related Acceptance Cases`: `DIFF-002`, `DIFF-003`, `DIFF-005`
...
- `Surface`: `diff-view`
- `Trigger`: 服务端返回 `DocDiff` 与 conflict protocol error
- `Preconditions`: `op.sc.merge-peer.request` 已发送且 remote/local 无法直接合并
- `Immediate Result`: 打开 diff/conflict surface，等待用户后续选择
- `Application Entry`: `apps/cli/src/server/handlers/merge/peer.rs`, `apps/web/src/hooks/use_core/effects/message_dispatch_projection_doc.rs`, `apps/web/src/hooks/use_core/effects/message_protocol.rs`
...
2. `Application Response`: 服务端发送 `DocDiff` 和 conflict error，前端切入 diff/conflict surface。
3. `Concrete Modules`:
   - `apps/cli/src/server/handlers/merge/peer.rs`
   - `apps/web/src/hooks/use_core/effects/message_dispatch_projection_doc.rs`
...
- conflict 出现后，后续如何选择 `KeepFs / KeepLedger / Accept Both` 仍由单独的 conflict flow 负责。
