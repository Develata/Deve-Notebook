...
- `Preconditions`: native shell mode, process/embedded supervisor, recovery bootstrap, and lifecycle gates remain explicit and default workspace build is unchanged
- `Immediate Result`: LocalBackend default and RemoteBrowser HTTPS shell progress are visible without giving native shell direct business authority
...
- Main objects: `platform::maturity`, `native::adapter_boundary`, `native::shell_mode`, `native::service_recovery`, `release::channel`, `release::artifact`.
