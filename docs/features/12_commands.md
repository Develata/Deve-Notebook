...
- 当前 backend/CLI 已接线 WebDAV push/pull 与 AWS `s3://` S3 push/pull；`s3+https://` custom endpoint 在显式 credential binding 完成前继续 fail-closed。
- Web Command Palette 入口不接收 locator 或 credential material；backend 使用当前 local repo 的
  `repo_url` 作为 Remote Projection locator。若 `repo_url` 不是所选 provider 的 transport URL，
  命令必须 fail-closed 并保留 `provider_io_ready=false`。
