...
- Android emulator 不稳定时使用独立手动 admission diagnostic：同一 exact HEAD 只构建一次 x86_64 debug APK，matrix 每次只改变 emulator binary source 或 system image，并固定重复三次 cold boot、guest-service 稳定准入、安装与 `system_server` 连续性检查。summary 会拒绝 APK、同 API system image 或 pinned emulator identity 漂移；主日志单文件和 variant 总上传量均有固定预算。其日志和 JSON 只用于选择后续 gate 配置，不是 receipt、candidate、签名或 tag-ready 证据，也不会自动修改正式 release gate。
