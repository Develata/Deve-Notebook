...
- Mobile pre-package compile/test 只能读取 clean checkout 中受版本控制的输入；被忽略的 Tauri/Wry Android generated source 只能在后续 package build 中物化。仓库单元合同只校验受控 ProGuard/JNI keep rule；同一 exact-HEAD/architecture 的 minified release-variant APK 必须经 ephemeral diagnostic signer 后在 owned emulator 上真实安装并保持启动，才能证明生成类、R8 与 JNI 的联合可用性。release proof 结束时必须验证其 package、launcher 与 app process 已全部退役；随后仅由被选中的 LocalBackend 或 RemoteBrowser journey 唯一负责安装并首次启动 debug APK、继续承担 CDP writable journey。外层编排不得预装同一 debug APK 后再让 journey 执行 `adb install -r`。两类 APK 与证据不得互相冒充，也不得把诊断 signer 写成候选签名身份。
