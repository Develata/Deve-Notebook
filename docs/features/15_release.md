...
- Android emulator 不稳定时使用独立手动 admission diagnostic：同一 exact HEAD 只构建一次 x86_64 debug APK，matrix 固定 pinned emulator 与 API 37 system image，每次只改变 `swangle / software / swiftshader` renderer mode，并固定重复三次 cold boot、guest-service 稳定准入、安装与 `system_server` 连续性检查。每轮都从有界 emulator 日志解析唯一的实际 Vulkan/GLES renderer pair；summary 会拒绝 APK、system image、pinned emulator identity、renderer 声明或三轮实际 pair 漂移，也不会把与不稳定 control 实际 pair 相同的 alias/fallback 推荐为修复。主日志单文件和 variant 总上传量均有固定预算。其日志和 JSON 只用于选择后续 gate 配置，不是 receipt、candidate、签名或 tag-ready 证据，也不会自动修改正式 release gate。
