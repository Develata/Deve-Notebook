...
| 顺序 | TODO | 优先级 | 范围 | 验收口径 |
|:--|:--|:--|:--|:--|
| 1 | Native process adapter decision | P3-10 / Decision | `apps/desktop/`, `apps/mobile/`, `docs/plan/08_ui_design_02_desktop.md`, `docs/plan/08_ui_design_03_mobile.md` | 在已落地 supervisor contract 基础上，决定真实 process adapter 是否进入当前 no-Tauri skeleton；若实现，必须只做受控 child-process/health-probe/session-handoff wiring，不打开 Tauri dependency gate，不写 core authority。 |
