...
- P1/P2 Git mirror queued export surface 已关闭：`deve_cli git export [--retry-out-of-sync]` 复用 explicit mirror executor，把 queued Deve projection commits 导出为 Git commits 并写回映射；完整 snapshot bootstrap export、import、push 仍保留为 future refinement。
