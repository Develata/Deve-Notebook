...
- DIFF-007: Diff 颜色语义。
- DIFF-008: 长文档打开策略。
- DIFF-009: Watcher 检测变更但不自动入 Ledger（写入 pending_fs_ops）。
- DIFF-010: Working Directory → Staging → Commit 三阶段工作流。
- DIFF-011: Commit 时对比 Ledger 快照生成 Ledger Facts。
- DIFF-012: .notegit/ 目录被 Watcher 忽略。
- DIFF-013: `.git/` mirror 内部路径被 Watcher 忽略，外部 Git 操作只进入 pending/import。
...
- UI-WEB-004: 浏览器刷新后按 `repo_id + branch` 恢复最近一次稳定 scope，名称仅作辅助恢复。
- UI-WEB-005: `Open in New Window` 保留现有 query params 并正确追加 `doc=...`。
- UI-DESK-001: 5 列网格与 Diff 分屏。
- UI-DESK-002: Sidebar/Panel 拖拽与持久化。
- UI-DESK-003: Unified Search 模式切换。
- UI-DESK-004: Activity Bar / Sidebar 的 `More(...)` 菜单项点击与 `Pin/Unpin` 语义分离。
- UI-MOB-001: 移动端视口映射。
...
- UI-DIFF-010: Unified Diff 支持折叠未变更区并支持点击展开。
- UI-DIFF-011: Diff 支持上下文行数 3/5/8 切换并即时生效。
- UI-DIFF-012: Diff 重算后语义锚点恢复，视口无明显跳变。
- UI-DIFF-013: Diff 头部显示缓存命中状态与最近计算耗时。
- UI-DIFF-014: Diff 内容变更后缓存键失效并触发重新计算。
- UI-DIFF-015: Diff 头部显示算法标签（Myers / Patience+Myers）。
- UI-DIFF-016: Diff 支持 F7 / Shift+F7 键盘导航变更块。
- UI-DIFF-017: Diff 头部显示缓存命中率并随操作更新。
- UI-DIFF-018: Diff 在不同 repo_scope 之间缓存隔离，避免跨仓命中。

## Search 搜索
- SEARCH-001: Standard + `search` feature 下全文搜索返回当前 repo scope 的 baseline scan 结果。
- SEARCH-002: LowSpec 或未启用 `search` feature 时全文搜索 fail-closed 并显示用户可见反馈。
- SEARCH-003: SearchResults stale request / repo / branch / scope 必须被前端丢弃。

## 网络连通
- NET-001: Web 端网络断连锁屏并进入重连态。
- NET-002: Main/Proxy 角色切换。
- NET-003: 协议格式区分。
- NET-004: WebLightPeer repo-scoped 握手回执包含 repo_id。
- NET-005: OpenDoc Snapshot-First。
- NET-006: Vector Gossip 缺失 Ledger Facts。
- NET-007: Snapshot Sync 覆盖模式。
- NET-008: 多仓库切换重新握手并隔离状态。
- NET-008A: `SwitchRepo / SwitchBranch` 的 `switch_nonce` 严格大于当前 `scope_nonce`。
- NET-009: 恶意数据隔离。
- NET-010: 间接同步信任边界。
- NET-011: WebSocket 错误必须走结构化 ProtocolError。
- NET-012: session expired 与 disconnected 明确分离。
...
- STORE-009A: `Remote -> Local` 返回时优先恢复最近一次稳定本地 repo，失败后再走 Fail-Closed。
- STORE-010: 路径规范化。
- STORE-011: Core Data Model: DB = OrderedLog<LedgerEntry>。
- STORE-012: pending_fs_ops 表存储未确认变更。
- STORE-013: .notegit/ 元数据目录（config/hooks/refs）。
- STORE-014: `.notegit/` 与 `.git/` 可共存，`.gitignore` 忽略 `.notegit/`。
...
- AUTH-011: session expired 与 disconnected 分离。
- AUTH-012: `/api/auth/status` 公开 session probe 不产生未登录 401 噪音。

## AI Agent
- AI-001: Native AI Chat 支持读取当前 Markdown 并完成基础 chat。
- AI-002: Native AI Chat 默认不暴露 MCP / Skills / Shell。
- AI-003: Trusted External Agent 默认关闭。
- AI-004: 未满足 trusted 条件时不得启动外部 CLI。
- AI-005: `/plan` 进入原生 PLAN 模式且禁止任何工具调用。
- AI-006: `/build` 进入原生 BUILD 模式；Markdown 写入只能通过 BUILD 下的受控 apply / edit 路径。
- AI-007: `/agents` 在原生 `PLAN ↔ BUILD` 间顺序切换。
- AI-008: BUILD 模式若使用程序执行，必须走受控执行路径，不得等价于通用 shell。
- AI-009: Native `PLAN / BUILD` 与 `Backend(native / trusted-cli)` 明确分离。
- AI-010: `AI: Switch Backend` 仅在 `enabled + trusted + AGENT_CLI_PATH` 条件满足时可见或可用。
- AI-011: `ai.mode=trusted-cli` 条件不满足时必须自动回退到 `native`，并在 chat 中给出明确原因。

## I18n
- I18N-001: UI 文本不硬编码。
- I18N-002: Locale 检测优先级。
- I18N-003: 不支持 Locale 回退。
- I18N-004: 后端仅返回错误码。
- I18N-005: 本地化格式。
- I18N-006: `/api/sc/*` 返回结构化错误码 JSON。

## 插件
- PLUG-001: Trusted External Agent 接口位已定义。
- PLUG-002: Calculation Runtime 接口位已定义。
- PLUG-003: Ledger-Managed Boundary 明确。
- PLUG-004: 当前插件章不要求代码实现。
...
- CMD-005: AI 模式切换。
- CMD-006: AI backend 切换仅在 trusted 条件满足时可见或可用。
- SET-001: 环境变量默认值。
- SET-002: `config.toml` 配置可由 CLI 更新。
- SET-003: `ai.mode=trusted-cli` 的回退语义正确。
...
- REG-MOB-025: 边缘滑动手势不吞掉 `File tree` / `Toggle Outline` 等靠边按钮点击。
- REG-MOB-026: 移动端 `More(...)` 条目点击切换视图，`Pin/Unpin` 仅修改固定状态。

## Source Control 与版本管理

- SC-001: Commit 锚定到 ledger_seq。
- SC-002: Commit 包含 parent_id（支持链式历史）。
- SC-003: 支持查看 Commit diff（对比任意两个 commit）。
- SC-004: Staging 区与 Working Directory 分离。
- SC-005: WebSocket 实时通知 FsChangeDetected。
- SC-006: 前端显示 Changes 列表（来自 pending_fs_ops）。
- SC-007: Git mirror commit 映射 Deve commit，失败进入 `GitMirrorOutOfSync` 且不回滚 ledger。
- SC-008: Git import/export/push/status 只能作为 bridge 命令，不得让 `.git/` 成为 authority。
- SC-009: `git import --apply` 只能写 pending/import；resolved stage/commit 后 `git export` 才能把该 Deve commit 映射为 Git commit。
- SC-010: `git push` 必须 fail-closed 于未导出的 queued/out_of_sync mirror record，不得创建或更新远端分支。
- SC-011: `git push` 必须 fail-closed 于 dirty Git worktree；清理 dirty path 后才允许发布已映射 Git HEAD。
