...
- `GitMirrorOutOfSync` 当前暴露 CLI-only `GitMirrorRepairAction` schema，把 failure stage / legacy error 映射为 repair action code、subject 与 retryable-after-fix 标记；它只用于诊断与显式 CLI retry 指引，不自动执行 Git。
