...
| **Git Ecosystem** | **First-class mirror bridge** | Partial (Mirror Queue Foundation) | `.notegit/` 保持 authority；`.git/` 作为生态镜像层。当前已落地共存/忽略/status 与 lazy `git_mirror_commits` queue/status side table；真实 Git executor、export/import/push 仍属 P1/P2 后续。 |
...
- 真实 Git mirror commit executor、retry/repair 执行闭环、import/export/push 仍是后续实现，不得被当前 queue/status 骨架替代。
