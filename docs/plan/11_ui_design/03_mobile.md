...
*   `LocalBackend` 必须复用 server native-session bridge 完成 session handoff，并以 HttpOnly native session cookie 与 `window.__DEVE_NATIVE_BOOTSTRAP` endpoint payload 启动 Web；bootstrap source 不得包含 token、secret 或 auth material。初次 WebView session prepare 与 resume/replacement handoff 必须共享同一个 process-local single-flight gate；同 generation 的重复 prepare 只能串行复核，不能并发竞争 Android `CookieManager` completion registry。用于阻止页面 reload 重复 prepare 的 browser marker 必须绑定当前 process/session 的非敏感安装身份；不得只绑定可能被新进程复用的 loopback endpoint，也不得让旧进程保存的 bootstrap 覆盖当前 fallback。保存的 bootstrap 必须通过 ready-shape、loopback endpoint 与稳定 capability 校验；同一 WebView process 后续 generation 的 endpoint 只能由 supervisor 的 replacement source 覆写，页面 reload 必须优先采用该同 process 保存值而不是 init script 中已过时的首 generation fallback。Android document-start 尚未取得 origin-scoped `sessionStorage` 时，只能在同一个初次 session admission 总窗口内重试 bootstrap storage；一次读写抛错，或一次成功 `setItem` 后尚未读回当前完整 envelope，均不构成 terminal identity 结论。每次重试都必须重新写入并读回 current install identity 与完整 ready bootstrap，且 native prepare 在精确确认前保持零次提交。总窗口耗尽、install identity 语法无效，或窗口内始终不能写后读确认时必须投影 `session_invalid`，不得调用 native handoff 或 reload。已提交的平台写入若超时，仍须保留 fail-closed tombstone 直至迟到 callback 到达或进程重启。
*   Tauri `main` WebView **MUST** 延迟到 embedded service 完成 probe、native session handoff 与 bootstrap plugin 注册之后创建；不得先创建无 endpoint/session bootstrap 的主 WebView。Android Wry 不实现 `WebView::set_cookie`，因此 Android 必须在 WebView 已登记后通过无参数 native command 调用系统 `CookieManager` 安装 HttpOnly cookie，确认成功后才 reload 一次并进入 authenticated runtime；document-start handoff 必须先在固定总窗口内确认 bootstrap storage，再确认 `__TAURI_INTERNALS__.invoke` 为真实函数，并轮询无副作用、bundled-origin 校验的 typed command-route readiness probe。storage 与 command-route readiness 必须复用同一个 bounded single-flight admission 和同一 attempt budget；只有两者都成功才可 single-flight 提交一次 native session prepare。probe 缺失、路由尚未登记、临时 rejection 或 storage 写后读尚未可见可在窗口内重试，prepare 一旦开始则不得因 rejection、timeout 或迟到 completion 再次提交。一次 browser task 延迟、一次早期 storage 未确认或仅观察到 invoke 函数都不构成 admission 结论；窗口耗尽必须投影 `session_invalid`，退休全部 probe，并阻止迟到 probe 触发 prepare。该路径只可暴露 allowlist 中的进行中阶段，或 `bootstrap-storage-failed / bridge-readiness-failed / native-prepare-failed / session-marker-failed` 终态类别；不得把 install id、endpoint、cookie、token、secret 或底层异常写入诊断。`Secure` cookie 的 retention 复核必须使用与 `setCookie` 相同的 HTTPS secure loopback origin，不得用 HTTP URL 的发送资格过滤误判已完成的平台写入，真实 HTTP loopback session 可用性仍必须由 reload 后的 auth/readiness 闭环证明。cookie/token/secret 不得进入 command 参数、JavaScript 或 bootstrap payload。`RemoteBrowser -> LocalBackend` 新建的 WebView 在调用该 command 或执行 resume handoff 前，还必须等待 native coordinator 确认新 `main` 已登记、recovery anchor 已退休且 `LocalWindowCreated` 已记录；该一次性 admission 只能由 backend/native lifecycle 打开，Web 不得自行推断 Activity 稳定性。
...
*   Android target-host 的首次文档 Create 证据必须把 quiet window 准入得到的精确 `repo_id + scope_nonce` 作为不可变期望值贯穿到 arm，并只在精确 path、唯一稳定 target 与 `ready + 非空 repo_id + 正整数 scope_nonce` 完全匹配时派发一次完整 native touch。紧邻 target/identity 准入之后、arm 与接触之前还必须重新等待当代主文档 `visibilityState = visible`、`document.hasFocus() = true`、精确应用 Activity `resumed` 与本节定义的原生输入焦点共识同时连续稳定至少 `250ms`；CDP 可连接、搜索结果可见、writer scope ready 或仅有页面焦点不能替代 Android 当前输入目标，Activity/输入焦点状态采样失败、非规范/冲突记录或 document replacement 必须在接触前 fail-closed。该 touch 必须在同一触点保持固定 `50ms` 的有界非零接触时间后才发送 `touchEnd`，不得把相邻的零接触时长命令冒充真实 tap。`touchEnd` 命令完成不等价于浏览器已同步合成 `click`；harness 必须绑定本次 arm token，在不重发 Create 的前提下以与 touch-transport lease 对齐的固定 `5000ms` deadline 有界等待同一 click settlement，并以最终原子读取与 observer 清理的结果作结算。页面 observation 只记录固定、无敏感信息的 `touchstart / touchend / pointerdown / pointerup / click` 阶段布尔值，不得记录路径、文档内容、session、endpoint 或 credential；阶段只用于定位 transport/gesture/click 边界，不能替代精确 target、writer scope 或 click 成功条件。arm 时先安装页面侧 touch-transport lease，`touchEnd` 返回后再切换为 click-settlement deadline；二者到期都必须自行 expiry/seal，使宿主 finalize 在执行前断连时仍能阻断迟到 click。迟到 click 的诊断延迟必须由页面接收 `touchend` 与 `click` 时各自采样的同一 wall-clock 域之差派生，不能用浏览器可能复用的 DOM `event.timeStamp`；只可投影有界相对毫秒数，不得输出绝对时间。该 Create lane 在本次 document lifecycle 中是 single-use：成功后结束；错误 target、writer scope 变化、missing observation、driver error、cleanup error 或 settlement timeout 一旦进入 committed-unknown，必须封存并阻断后续迟到 click，只有新的页面 generation 才能重新准入，不得用第二次 Create 掩盖 committed-unknown。
...
*   CLI native runtime 必须提供 owned `EmbeddedServerRuntime`、内部 transport graceful-shutdown 入口与 typed shutdown coordinator。每个 active transport generation 都必须拥有一个未与其它 generation 共享的 coordinator；正常 app exit 必须把该同一 coordinator 同时交给 transport 与随后 runtime owner cleanup，使二者消费同一 absolute deadline。replacement 候选必须携带自己的 coordinator，并只在 generation 提交时替换当前 coordinator；不得以 transport 剩余 `Duration` 重新启动 runtime timeout。超时或 join/runtime error 必须进入结构化 error 状态，不能报告 clean shutdown。
*   replacement transport 从 spawn 到 commit/retire 期间必须登记为 supervisor-owned pending generation，不能只存在于 resume future 的局部变量。shutdown 必须先取得 WebView handoff gate；在 absolute deadline 前无法取得时只能返回 typed timeout、不得拆除仍被 handoff 使用的 authority runtime。supervisor Drop 仍必须同步发出 pending/current transport cancellation 并 abort 对应 task，使 candidate 不会脱离 runtime owner 存活。取得 gate 后，transport join、resume drain 与 runtime cleanup 必须复用 coordinator 返回的同一 effective deadline。
...
*   **Target-host Input Admission**: Android target-host 通过 ADB 注入 Drawer 手势前，必须等待当代主文档
    ...
    `resumed` Activity，且 WindowManager `mCurrentFocus` 与 InputDispatcher 当前 focused window 按本节规则形成
    精确、无冲突的同应用输入焦点共识，并连续稳定至少
    ...
    已观测的 `deve-native-back-request` 到达 Web 并将其归一到 closed，再重新要求完整 closed 几何稳定后才能接触屏幕。
    ADB keyevent 必须先在页面内 arm 一次性、token 绑定且有界的 Back delivery observation；只有 observation 证明
    本次 keyevent 完全没有到达 Web、Drawer 仍语义 open 时，才允许在重新完成当代 WebView 输入焦点准入后补发至多
    一次 transport-level keyevent。任一 Web Back 已到达后都不得补发；`Unhandled`、无效 ack、已到达但 Drawer 未关闭、
    ...
    若 LocalBackend Drawer swipe/Back lane 的短时被动准入未取得完整输入目标，target-host 可在记录当前有效
    PID 后执行至多一次既有 Launcher foreground reentry；该动作只能恢复 Android task/window ownership，不能重建业务
    surface、重发 swipe 或 Back。Create lane 与 RemoteBrowser HTTPS WebView **MUST NOT** 使用 foreground reentry，
    必须在完整被动窗口内取得输入目标或 fail-closed，避免重新启动 Activity intent 扰动已绑定的 remote Document。
    Drawer reentry 后必须从零重新证明当代 Document、resumed Activity 与原生输入焦点共识的完整稳定
    窗口，并要求 PID 与 reentry 前完全一致；PID 替换、driver 失败或第二个准入窗口耗尽都必须 fail-closed。
    失败诊断只允许输出 reentry 是否执行、PID 是否稳定及页面、Activity、WindowManager、InputDispatcher、
    输入焦点共识与最终 target 的固定分类，不得输出原始
...
*   target-host 对 ADB 输入窗口的读取必须分别解析 WindowManager 规范 `mCurrentFocus` 与 InputDispatcher
    规范 focused window：后者接受当前 AOSP 的 `FocusedDisplayId + FocusedWindows/displayId/name`、无
    `FocusedDisplayId` 时唯一的 single-display `FocusedWindows` 记录，并兼容旧式唯一 `FocusedWindow: name=`
    记录；若 `dumpsys input` 同时携带 `Input Dispatcher State at time of last ANR`，该历史快照必须排除，不能与
    当前区段混合或替代当前输入权威。两条证据都必须按精确 package component 分类。探针命令失败、输出缺失或
    平台明确报告 `null` / `<none>` 属于 soft-unavailable；仅这种不可用状态可由另一条精确、无冲突的原生 authority
    提供 focused/not-focused 结论。不可解析记录、非规范或负 display ID、同一 authority 多个 component 冲突、
    package 前缀碰撞、current/history 区段歧义或其它格式/身份完整性失败属于 hard-invalid；任一探针 hard-invalid
    必须整体阻断，不能由另一条精确结果覆盖。两个精确探针可用时必须结论一致；冲突必须整体 unavailable。
    单探针精确 focused 只有在精确 Activity 同时为 resumed 且另一探针仅 soft-unavailable 时才可准入；两个探针都
    soft-unavailable 时必须阻断。对外诊断仍将 soft-unavailable 与 hard-invalid 都投影为固定 `unavailable`，不得
    输出原始窗口或错误 detail。Activity resumed 与原生输入焦点共识必须在同一采样窗口同时成立，
    任一变化都从零重新累计 `250ms`，不得把页面焦点、Activity 状态、任一单独的 unavailable 探针或 package
    前缀匹配冒充完整输入目标准入。诊断只能投影 Activity、WindowManager、InputDispatcher、共识与最终 target 的
    固定分类，不得输出原始 `dumpsys` 或窗口名。
