...
- Mobile packaging dependency spike 已打开：`tauri` / `tauri-build` **MAY** 只作为 `apps/mobile`
  的 optional dependency 存在，并且必须挂在 `native-packaging` feature 后。
- Mobile packaging scaffold 只记录 WebView shell/permission bridge/share sheet/deeplink/file picker/push notification/store package acceptance；Android shell-only package execution 可由 `11_ui_design_03_mobile.md#mobile-android-shell-package-execution-gate` 单独打开。
- iOS shell-only package execution 可由 `11_ui_design_03_mobile.md#mobile-ios-shell-package-execution-gate` 单独打开；Mobile runtime entrypoint、process runtime、native authority write path 与 release ready 不得由 Android/iOS package execution 隐式打开。
