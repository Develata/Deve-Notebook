...
- 已实现 `deve_cli git import` 只读 dry-run planning：检查 mirror readiness、Git worktree、`.notegit` tracked 泄漏与 Git HEAD，解析 tracked/untracked worktree changes 为 `GitImportPlan` change/blocker；当前不写 ledger、pending_fs、staging 或 `.notegit`，真正 pending/import apply 与 push 仍是后续。
