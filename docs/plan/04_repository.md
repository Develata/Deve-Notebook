...
- 本轮唯一compiled producer是server composition内的typed `RepoLifecycleCoordinator::readmit_retired_repo`，它只能在调用方已拥有exact RepoId、live Retired proof与Transitioning permit时调用core owner API；integration producer走同一非test实现。F4/v6 `RepoLifecycleIntent::Create`继续只生成fresh RepoId，Remote Import也不伪装成readmission；本轮不新增WS/CLI/UI入口。未来若产品要让用户显式跨重启导入既有RepoId，必须另行批准typed transport与durable lineage proof，前端仍只能提交RepoId intent，不能持有authority/identity/activation能力。
