...
- `EmbeddedServerRuntime` **MUST** own cancellation and join handles for every background task it starts. Axum listener shutdown alone is not a complete runtime shutdown；normal native app exit 必须先停止 active transport generation，再在一个 process-scoped 总截止时间内 cancel/join metrics、prewarm、P2P connector、repo lifecycle worker，并释放 watcher ownership。composition root 必须为 active transport generation 与随后 process owner cleanup 持有并传递同一个 typed shutdown coordinator；各阶段只能消费该协调器已开始的总截止时间剩余量，不得只传递重新计算的 `Duration` 或分别获得一份完整 timeout。transport replacement 必须为新 generation 创建尚未开始的独立 coordinator，旧 generation 的已开始截止线不得污染新 generation。Mobile replacement candidate 从 spawn 到 commit/retire 必须登记在 supervisor 内，不能只由 resume future 局部拥有；handoff gate 超时不得拆除仍在使用的 authority runtime，最终 Drop 必须仍能取消 pending candidate。
