...
*   **Baseline CLI Contract**:
    ...
    *   `deve repo removal-repair --request-id <uuid>`: 默认只读，输出exact remaining target categories、每类owner truth与可用时的短期repair token；不暴露host path/digest。只有exact Removed cut、原sealed plan/checkpoint与全部remaining owner identity仍一致时才签发token；Changed/Unknown/unsafe reparse只报告typed blocker。
    *   `deve repo removal-repair --request-id <uuid> --apply --token <opaque>`: 在同一lifecycle worker内一次性消费五分钟token并重试原sealed committed cleanup；token绑定offline lifecycle receipt owner identity或proxy operator+server incarnation。consumed token必须可在响应丢失、worker运行中及终态后返回同一job/result，不重复cleanup。不得重写manifest、重绑定replacement或扩张target；unknown/mismatched/unsafe reparse永远fail-closed。
    *   offline `removal-repair`必须先取得persistent lifecycle receipt lock再初始化repo/scan/watcher，并排除目标request的startup auto-recovery；只有lock已由server持有时才读取owner hint并走authenticated proxy。stale hint不是authority证据。
    *   repo removal CLI进程退出码固定为：`0`成功或只读preview、`20`明确未提交、`21`已提交但仍有cleanup/publication debt、`22`需要repair；其它CLI错误仍为`1`。stdout只输出typed machine fields，stderr不得包含path、digest、token或raw owner detail。
