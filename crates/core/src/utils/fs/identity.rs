...
impl HostPathIdentity {
    ...
    pub fn revalidate(&self) -> std::io::Result<bool> {
        ...
        let parent_identity = identity_for(parent, HostPathKind::Directory)?;
        let object_identity = identity_for(&self.path, self.kind)?;
        Ok(parent_identity == self.parent_identity && object_identity == self.object_identity)
    }
...
fn identity_for(path: &Path, kind: HostPathKind) -> std::io::Result<HostFileIdentity> {
    let file = open_no_follow(path, kind)?;
    identity_from_handle(&file)
}

#[cfg(unix)]
fn open_no_follow(path: &Path, kind: HostPathKind) -> std::io::Result<File> {
...
fn open_no_follow(_path: &Path, _kind: HostPathKind) -> std::io::Result<File> {
...
#[cfg(unix)]
fn identity_from_handle(file: &File) -> std::io::Result<HostFileIdentity> {
...
#[cfg(not(any(unix, windows)))]
fn identity_from_handle(_file: &File) -> std::io::Result<HostFileIdentity> {
    Err(std::io::Error::new(
        std::io::ErrorKind::Unsupported,
...
#[cfg(test)]
mod tests {
    use super::{HostPathIdentity, HostPathKind};
