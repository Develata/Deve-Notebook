...
use crate::ledger::schema::{
    CLIENT_OP_INDEX, DOC_OPS, DOCID_TO_PATH, INODE_TO_DOCID, INODE_TO_NODEID, LEDGER_OPS,
    MERGE_BASE_CHECKPOINT, NODE_OPS, NODEID_TO_META, PATH_TO_DOCID, PATH_TO_NODEID, PEER_FACT_OPS,
    PEER_FACT_SEQ, SNAPSHOT_DATA, SNAPSHOT_INDEX,
};
...
impl<'a> ShadowMaintenanceRuntime<'a> {
    ...
    pub(crate) fn reset_shadow_doc(
        ...
        let _guard = self.manager.lock_shadow_merge()?;
    ...
    pub(crate) fn reset_shadow_repo_txn(write_txn: &redb::WriteTransaction) -> Result<()> {
        ...
        let _ = write_txn.delete_table(PEER_FACT_SEQ)?;
        let _ = write_txn.delete_table(PEER_FACT_OPS)?;
        let _ = write_txn.delete_table(MERGE_BASE_CHECKPOINT)?;
