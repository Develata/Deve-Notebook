...
fn watcher_startup_root_revalidation_does_not_create_changed_locator_root() -> anyhow::Result<()> {
    ...
    repo.set_projection_base_for_local_repo("default", &old_base)?;
    let old_root = repo.local_repo_workspace_root("default")?;
    std::fs::create_dir_all(&old_root)?;
    let old_root = std::fs::canonicalize(old_root)?;
    let vfs = Vfs::new(&old_root);

    repo.set_projection_base_for_local_repo("default", &new_base)?;
    let changed_root = repo.local_repo_workspace_root("default")?;
    assert!(!changed_root.exists());
    let repo = Arc::new(repo);

    scan_local_repo_at_root(&repo, &vfs, "default", &old_root)
...
fn watcher_startup_scan_file_never_reresolves_the_locator() -> anyhow::Result<()> {
    ...
    repo.set_projection_base_for_local_repo("default", &new_base)?;
    let new_root = repo.local_repo_workspace_root("default")?;
    std::fs::create_dir_all(&new_root)?;
    std::fs::write(new_root.join("note.md"), "new-root-content")?;
    let repo = Arc::new(repo);

    super::super::scan_file::scan_disk_file(&repo, &vfs, "default", "note.md", &old_file)?;
    let pending = repo
        .run_on_local_repo("default", |db| pending_fs::get(db, "note.md"))?
