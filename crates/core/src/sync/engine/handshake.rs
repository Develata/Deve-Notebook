...
impl SyncEngine {
    ...
    pub fn compute_diff(&self, remote_vector: &VersionVector) -> (Vec<protocol::SyncRequest>, Vec<protocol::SyncRequest>) {
    ...
    pub fn handshake(
        ...
        if remote_peer_id.as_str() != derived_id {
            return Err(anyhow!("PeerID mismatch: claimed {}, derived {}", remote_peer_id, derived_id));
        }

        // 2. Verify Signature
        // Message = "deve-handshake" + peer_id + json(vector)
        // 注意: 必须保证序列化确定性。这里简单使用 serde_json，生产环境建议使用 canonical serialization。
        let vec_bytes = serde_json::to_vec(&remote_vector)?;
        let mut msg = Vec::new();
