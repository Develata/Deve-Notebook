...
pub(super) fn init_table(write_txn: &WriteTransaction) -> Result<()> {
    let _ = write_txn.open_multimap_table(PENDING_FS_DOC_INDEX)?;
    Ok(())
}
...
pub(super) fn replace(
    ...
    let mut table = write_txn.open_multimap_table(PENDING_FS_DOC_INDEX)?;
...
pub fn paths_for_doc(db: &Database, doc_id: DocId) -> Result<Vec<String>> {
    ...
    let table = read_txn.open_multimap_table(PENDING_FS_DOC_INDEX)?;
