...
//!
//! F4/v5 Remote Import wire contract tests.
...
use crate::protocol::{
    ClientMessage, LocalRepoRemovalPreservedCategory, LocalRepoRemovalPreview,
    LocalRepoRemovalWarning, OpaqueFallbackBinding, RemovalConfirmationToken, RepoControlRequest,
    RepoControlResponse, ServerErrorCode, ServerMessage, SwitchNonce,
};
...
#[test]
fn remote_import_and_repo_control_nested_wire_roundtrip_in_f4_v5_binary_and_versioned_json() {
    for request in request_variants() {
        ...
        let frame = decode_client_binary_frame(&binary).expect("decode F4/v5 client frame");
