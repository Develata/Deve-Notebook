...
#[test]
fn list_docs_prefers_node_projection_over_stale_metadata_path() {
    ...
    assert_eq!(
        repo.list_local_docs(Some(repo.local_repo_name()))
            .expect("list local docs"),
        vec![(doc_id, "notes/a.md".to_string())]
    );
    ...
    let repo_id = repo
        ...
        .uuid;
    assert_eq!(
        repo.list_docs(&RepoType::Local(repo_id))
            .expect("list docs through trait"),
        vec![(doc_id, "notes/a.md".to_string())]
    );
}
