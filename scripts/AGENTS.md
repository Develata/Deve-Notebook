...
| `lib/android-document-create-flow.mjs`; `android-document-create-flow.test.mjs`; `android-document-create-pointer.test.mjs` | Drives and tests one path-bound pointer Create intent, uses a capture guard to accept same-path remounts and block wrong targets, then carries the unique backend-projected OpenDoc `doc_id` into exact writable-editor admission without retrying Create |
| `lib/android-webview-pointer.mjs`; `android-webview-pointer.test.mjs` | Owns and tests the complete CDP primary-button gesture plus a before-press canonical point replacement hook shared by Android WebView target-host journeys |
...
| `lib/mobile-webview-interaction.mjs` | Focus, doc-id-bound editor-session admission, text-input failure diagnostics, and mobile drawer navigation helpers for the Android WebView lifecycle harness |
