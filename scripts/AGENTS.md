...
| `lib/android-emulator-boot-readiness.sh` / `android-emulator-boot-readiness.test.sh` | Requires bounded read-only package-manager and settings-provider readiness before APK installation, with exact fail-closed regressions for incomplete Android system-provider bootstrap |
