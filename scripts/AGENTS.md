...
| `lib/android-document-create-flow.mjs`; `android-document-create-flow.test.mjs` | Drives and tests one path-bound pointer Create intent with post-move identity and DOM-click observation, then carries the unique backend-projected OpenDoc `doc_id` into exact writable-editor admission without retrying Create |
| `lib/android-webview-pointer.mjs`; `android-webview-pointer.test.mjs` | Owns and tests the complete CDP primary-button gesture and before-press revalidation hook shared by Android WebView target-host journeys |
