...
| `lib/android-business-flow.mjs`; `android-business-flow.test.mjs` | Shared Android WebView UI-intent helpers and regressions for repo bootstrap, document edit, and Source Control commit/history; they own no authority and are reused by LocalBackend and RemoteBrowser smokes |
| `lib/android-document-create-flow.mjs`; `android-document-create-flow.test.mjs` | Drives and tests one path-bound, stable pointer Create intent, then carries the unique backend-projected OpenDoc `doc_id` into exact writable-editor admission without retrying Create |
