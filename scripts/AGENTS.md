...
| `lib/android-document-create-flow.mjs`; `android-document-create-flow.test.mjs`; `android-document-create-pointer.test.mjs` | Drives and tests one path-bound native-touch Create intent, uses a capture guard to accept same-path remounts and block wrong targets, then carries the unique backend-projected OpenDoc `doc_id` into exact writable-editor admission without retrying Create |
