...
| `check-mobile-android-emulator-install-startup-smoke.sh` | Owns an Android emulator target lifecycle, admits a minified release-variant APK through the shared stable single-PID readiness boundary, then delegates the debug APK's only install/start to one explicit LocalBackend or RemoteBrowser writable journey before bounded cleanup |
