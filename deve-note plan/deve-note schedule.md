...
- [x] **存储层 A (Ledger - Redb)**:
    - [x] 实现 `OpSeq` 键值存储设计 (Zero-copy).
    - [x] 集成 Loro CRDT (Text/Map/List) 并封装为 `Doc` 对象.
- [x] **存储层 B (Vault - FS)**:
    - [x] 实现 `notify` 文件监听器 (Watcher).
    - [x] 实现文件变更 **防抖 (Debounce)** 逻辑 (>500ms).
- [x] **和解循环 (Reconciliation Loop)**:
...
- [x] **应用外壳 (App Shell)**:
    - [x] **5列网格布局**: Sidebar | Diff(Old) | Diff(New) | Outline | Chat.
    - [x] **拖拽调整**: 实现各列宽度的拖拽调整 (Resizer).
    - [x] **状态栏**: 独立的 Flex 布局 (底部).
...
- [x] **统一搜索模态框 (Unified Search Modal)**:
