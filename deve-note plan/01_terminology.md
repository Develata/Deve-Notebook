...
*   **Ledger (账本)**：系统的唯一真值源（Source of Truth）。
    ...
    *   定义为一个不可变的账本事实序列 $L = [Fact_1, Fact_2, ..., Fact_n]$。
...
*   **Projection (投影)**：从 Ledger 派生的、面向用户的可读/可编辑形式（如 Markdown 文件）。
    *   **Description (描述)**：这是呈现给用户看的当前状态。用户对它的修改不会直接生效，而是必须转化成新的 Ledger Facts 追加到 Ledger 中。
...
*   **LedgerSeq (账本序列数)**：Peer 维度的单调递增计数器。
    *   **Definition**: $Seq(P, i) \in \mathbb{N}$ (Implementation: `u64`)，表示 Peer $P$ 产生的第 $i$ 条账本事实。
    *   **Property**: 单调递增，决定账本事实的全序关系。
